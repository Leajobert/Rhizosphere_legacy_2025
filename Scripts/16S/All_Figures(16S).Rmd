---
title: "Script Analyses SBL Cambodge"
output: html_document
date: "2025-11-12"
---

# Project information
**Project:** Projet Léa Jobert - exp SBBL Cambodge  
**Experiment:**  
Changes in the microbiota associated with diseased rice plants in Cambodia:  
What is the legacy for growth and resistance of the next generation?  
**Plant species:** Rice var. Srangae Sral  
**Compartments:** Rhizosphere, root, leaf, soil  
**Inoculation:** Xoo, XOC, and field manipulation  
Amplicon sequencing of *16S* and *ITS*
...
```{r}
# Load CSV containing ASV, taxonomy and abundance data
file_path <- "your_path/16S/SBL_16S_tout_R.csv"
df <- read.csv(file_path, sep=";", header=TRUE, stringsAsFactors=FALSE)

# Create taxonomy table (ASVs as rownames)
taxa <- df[, c("ASV", "Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")]
rownames(taxa) <- taxa$ASV
taxa$ASV <- NULL
saveRDS(taxa, "your_path/16S/datataxamodifie.RDS")

# Create ASV table (samples in rows, ASVs in columns)
asv <- df[, setdiff(names(df), c("taxonomy", "ASV", "Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species"))]
rownames(asv) <- asv$ID
asv$ID <- NULL
asv <- t(asv)
colnames(asv) <- df$ASV
saveRDS(asv, "your_path/16S/dataASVmodifie.RDS")

# Load RDS files and metadata
taxa <- readRDS("your_path/16S/datataxamodifie.RDS")
metadata <- read.csv2("your_path/16S/metadataSBL.csv")
asv16S <- readRDS("your_path/16S/dataASVmodifie.RDS")

# Match ASV table and metadata
rownames(metadata) <- metadata$SampleID
keep.cols <- c("SampleID", "Generation", "Initialsoil", "Compartment", "Condition", "Group", "Status", "Compartment_Status")
samples.out <- rownames(asv16S)
metadata <- metadata[metadata$SampleID %in% rownames(asv16S), keep.cols]
rownames(metadata) <- metadata$SampleID

# Convert ASV table to numeric and reassign rownames
asv16S <- apply(asv16S, 2, as.numeric)
rownames(asv16S) <- samples.out

# Create phyloseq object
otu_tab <- otu_table(as.matrix(asv16S), taxa_are_rows=FALSE) 
taxa_tab <- tax_table(as.matrix(taxa))
ps16S_1 <- phyloseq(otu_table(asv16S, taxa_are_rows=FALSE), 
...
ps16S_1 <- subset_taxa(ps16S_1, Kingdom != "k__Viridiplantae")

# Remove missing Kingdom and Phylum
ps16S_2 <- subset_taxa(ps16S_1, !is.na(Kingdom) & !is.na(Phylum))

# Remove mitochondria and chloroplasts
ps16S_3 <- subset_taxa(ps16S_2, Family != "Mitochondria" | is.na(Family) & Class != "Chloroplast" | is.na(Class))
ps16S_4 <- subset_taxa(ps16S_3, !is.na(Kingdom) & !is.na(Phylum) & !Order %in% c("Chloroplast") & !Family %in% c("Mitochondria"))

# Final filtered object
ps16S <- ps16S_4

# Remove specific conditions
ps16S <- subset_samples(ps16S, !(Condition %in% c("Diseased2", "Xoc")))
ps16S <- prune_taxa(taxa_sums(ps16S) >= 1, ps16S)

# Optional: higher abundance threshold
ps16S10 <- prune_taxa(taxa_sums(ps16S) >= 10, ps16S)
...
##Figure 1A - Alpha Diversity (16S) - Generation 0
```{r}
library(ggpubr)
library(cowplot)

# Subset Generation 0
ps16Sgen0 <- subset_samples(ps16S, Generation == "Gen0")
ps16Sgen0 <- prune_taxa(taxa_sums(ps16Sgen0) >= 1, ps16Sgen0)

# Calculate Observed richness
alpha_div <- estimate_richness(ps16Sgen0, measures = c("Observed")) %>%
  mutate(
    SampleID = rownames(.),
    Compartment = sample_data(ps16Sgen0)$Compartment,
    Status = sample_data(ps16Sgen0)$Status,
...
p <- ggplot(alpha_div, aes(x = Group, y = Observed, fill = Group, shape = Status)) +
  ...
  ggtitle("Alpha Diversity - Observed Richness (16S)") +
...
# Save
ggsave(
  filename = "your_path/16S/Figures/AlphaDiversity_plots_gen0/AlphaDiversity_Observed_Grouped_with_stats.svg",
  plot = p_stat,
  width = 9,
  height = 6,
  units = "in",
...
## Figure 1C-D-E - Beta Diversity (16S) - Generation 0
```{r}
## 16S - PCoA per compartment
## Figures 1D (Root), 1E (Rhizosphere), 1C (Leaf)

# Output directory
output_dir_16S_pcoa <- "your_path/PCoA_par_compartiment"
...
## Figure 1D - Root (16S)
ps_root <- subset_samples(ps16S, Compartment == "Root")
...
ggsave(file.path(output_dir, "PCoA_16S_Root_Fig1C.svg"), plot = p_root, width = 7, height = 5, device = "svg")


## Figure 1E - Rhizosphere (16S)
ps_rhizo <- subset_samples(ps16S, Compartment == "Rhizosphere")
...
ggsave(file.path(output_dir, "PCoA_16S_Rhizo_Fig1D.svg"), plot = p_rhizo, width = 7, height = 5, device = "svg")


## Figure 1C - Leaf (16S)
ps_leaf <- subset_samples(ps16S, Compartment == "Leaf")
...
ggsave(file.path(output_dir, "PCoA_16S_Leaf_Fig1E.svg"), plot = p_leaf, width = 7, height = 5, device = "svg")
...
```{r}
## Figure 2 - 16S Top 30 genera 
#script for Fig. 2 A) Rhizosphere 16S, same to be done with root (Fig 2. C)
...
# Output directory
output_dir <- "your_path/16S/Fig2_TaxonomicBinning"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

## 1. Subset to Generation 0 - Rhizosphere samples
ps16S_gen0 <- subset_samples(ps16S, Generation == "Gen0")
ps16S_gen0_rhizo <- subset_samples(ps16S_gen0, Compartment == "Rhizosphere")
ps16S_gen0_rhizo <- prune_taxa(taxa_sums(ps16S_gen0_rhizo) >= 1, ps16S_gen0_rhizo)

## 2. Agglomerate taxa to the Genus level
ps16S_genus <- tax_glom(ps16S_gen0_rhizo, taxrank = "Genus", NArm = TRUE)

## 3. Transform to relative abundances
ps16S_genus_rel <- microbiome::transform(ps16S_genus, "compositional")

## 4. Select the top 30 genera across all samples
genus_sums <- taxa_sums(ps16S_genus_rel)
top30_ids <- names(sort(genus_sums, decreasing = TRUE))[1:30]
ps16S_top30 <- prune_taxa(top30_ids, ps16S_genus_rel)
...
sample_data(ps16S_top30)$Condition <- factor(
  sample_data(ps16S_top30)$Condition,
  levels = c("Healthy", "Diseased1") # Healthy first
)

sample_data_df <- as.data.frame(sample_data(ps16S_top30))
new_names <- ifelse(
...
sample_names(ps16S_top30) <- new_names

## 7. Plot stacked bar chart of top 30 genera
p <- plot_bar(ps16S_top30, fill = "Genus") +
  ...
  ggtitle("Top 30 genera - 16S, Generation 0, Rhizosphere")

## 8. Save the figure
ggsave(
  filename = file.path(output_dir, "Taxonomic_Binning_Top30Genus_16S_Gen0_Rhizo.svg"),
  plot = p,
  dpi = 300,
  device = "svg",
  width = 12,
...
ps16S_gen0_sol <- subset_samples(ps16S10, Compartment %in% c("Rhizosphere", "Soil"))
ps16S_gen0_sol <- prune_taxa(taxa_sums(ps16S_gen0_sol) >= 1, ps16S_gen0_sol)
ps16S_gen0_solTSS <- transform_sample_counts(ps16S_gen0_sol, function(x) x / sum(x))

colors_gen0_sol <- c("G0-Rhizo-D1" = "#E97428", "G0-Rhizo-H" = "#67B85A", 
                     "MixsolH" = "#3A7231", "MixsolXoo" = "#CB5E15")

# Bray-Curtis distance
bray_dist <- phyloseq::distance(ps16S_gen0_solTSS, method = "bray")

# PCoA ordination
ordination_pcoa <- ordinate(ps16S_gen0_solTSS, method = "PCoA", distance = "bray")
ordination_data_pcoa <- plot_ordination(ps16S_gen0_solTSS, ordination_pcoa, 
                                        type = "samples", color = "Group", shape = "Compartment")$data

# Métadonnées et tests stats
metadata <- as(sample_data(ps16S_gen0_solTSS), "data.frame")
...
ggsave(file.path(output_dir_final, "Fig4_PCoA_ordination_champs_sol_16S.svg"), 
...
# subset "soil"
ps16S_soil <- subset_samples(ps16S10, Compartment %in% c("Soil"))
ps16S_soil <- prune_taxa(taxa_sums(ps16S_soil) > 0, ps16S_soil)


ps16S_soil_filtered <- phyloseq::filter_taxa(ps16S_soil, function(x) {
  prevalence <- sum(x > 0) / length(x)
  rel_abundance <- sum(x) / sum(otu_table(ps16S_soil))
...
ps16S_soil_genus <- tax_glom(ps16S_soil_filtered, taxrank = "Genus", NArm = TRUE)
dds <- phyloseq_to_deseq2(ps16S_soil_genus, ~ Condition)
...
tax_table_df <- as.data.frame(tax_table(ps16S_soil_genus))
...
otu <- as.data.frame(otu_table(ps16S_soil_genus))
if (!taxa_are_rows(ps16S_soil_genus)) {
  otu <- t(otu)
}
otu <- as.data.frame(otu)
otu$taxon <- rownames(otu)

tax <- as.data.frame(tax_table(ps16S_soil_genus))
...
write.xlsx(res_sig_full, 
           file = file.path(output_dir_sol, "ps16S_soil_genus_Deseq_Diseased1_vs_Healthy_significatifs.xlsx"), 
           rowNames = FALSE)

# 💾 Exporter en Excel
write.xlsx(res_all_full, 
           file = file.path(output_dir_sol, "ps16S_soil_genus_Deseq_Diseased1_vs_Healthy_tout.xlsx"), 
           rowNames = FALSE)
...
df <- read_excel("ypur_path/ps16S_soil_genus_Deseq_Diseased1_vs_Healthy_significatifs.xlsx")
...
ggsave(
  filename = "your_path/Fig4_log2FoldChange_barplot_sol_genre_16S.svg",
...
ps16S_sol_gen1 <- subset_samples(ps16S10, Compartment %in% c("Soil", "Rhizo"))
ps16S_sol_gen1 <- prune_taxa(taxa_sums(ps16S_sol_gen1) >= 1, ps16S_sol_gen1)
ps16S_sol_gen1TSS <- transform_sample_counts(ps16S_sol_gen1, function(x) x / sum(x))

colors_sol_gen1 <- c("G1-SoilXoo-Rhizo-Xoo" = "#D52F20", "G1-SoilH-Rhizo-Xoo" = "#3EBBA4",
                     "MixsolH" = "#3A7231", "MixsolXoo" = "#CB5E15")

# Bray distance
bray_dist <- phyloseq::distance(ps16S_sol_gen1TSS, method = "bray")

# PCoA
ordination_pcoa <- ordinate(ps16S_sol_gen1TSS, method = "PCoA", distance = "bray")
ordination_data_pcoa <- plot_ordination(ps16S_sol_gen1TSS, ordination_pcoa, 
                                        type = "samples", color = "Group", shape = "Compartment")$data

metadata <- as(sample_data(ps16S_sol_gen1TSS), "data.frame")
permanova <- adonis2(bray_dist ~ Group, data = metadata, permutations = 999)
pval <- formatC(permanova$`Pr(>F)`[1], format = "e", digits = 2)
r2val <- formatC(permanova$R2[1], format = "f", digits = 2)
...
ggsave(file.path(output_dir_final, "Fig5_PCoA_ordination_sol_g1_16S.svg"),
...
##Figure 7 A) and B) 16S
```{r}
## Figure 7 – Venn Diagrams (Genus level) for 16S data
...
ps16Ssoilrhizo_genre <- subset_samples(ps16S10, Compartment %in% c("Rhizo", "Soil", "Rhizosphere"))
ps16Ssoilrhizo_genre <- prune_taxa(taxa_sums(ps16Ssoilrhizo_genre) >= 1, ps16Ssoilrhizo_genre)
ps16Ssoilrhizo_genre <- tax_glom(ps16Ssoilrhizo_genre, taxrank = "Genus", NArm = TRUE)
...
g0_d1_genre   <- get_present_taxa_genre(ps16Ssoilrhizo_genre, "G0-Rhizo-D1", min_prevalence = 0.1, min_abundance = 0.0001)
soil_d1_genre <- get_present_taxa_genre(ps16Ssoilrhizo_genre, "MixsolXoo",    min_prevalence = 0.5, min_abundance = 0.001)
g1_xoo_genre  <- get_present_taxa_genre(ps16Ssoilrhizo_genre, "G1-SoilXoo-Rhizo-Xoo", min_prevalence = 0.5, min_abundance = 0.001)
...
# Save plot
ggsave(
  "your_path/Fig7_Venn_Genus_D1_16S.svg",
  venn_plot_d1,
  width = 5, height = 4.5
)
...
taxo_genres_shared_d1 <- extract_taxo_table_genre(ps16Ssoilrhizo_genre, genre_intersection_d1)
# Optional export:
# write_csv(taxo_genres_shared_d1, "Shared_Genus_G0_Soil_G1_D1.csv")

# 🔹 Repeat process for H groups
g0_h_genre   <- get_present_taxa_genre(ps16Ssoilrhizo_genre, "G0-Rhizo-H", min_prevalence = 0.1, min_abundance = 0.0001)
soil_h_genre <- get_present_taxa_genre(ps16Ssoilrhizo_genre, "MixsolH",    min_prevalence = 0.5, min_abundance = 0.001)
g1_h_genre   <- get_present_taxa_genre(ps16Ssoilrhizo_genre, "G1-SoilH-Rhizo-Xoo", min_prevalence = 0.5, min_abundance = 0.001)
...
# Save plot
ggsave(
  "your_path/Fig7_Venn_Genus_H_16S.svg",
  venn_plot_h,
  width = 5, height = 4.5
)
...
taxo_genres_shared_h <- extract_taxo_table_genre(ps16Ssoilrhizo_genre, genre_intersection_h)
...
ps_filtered <- prune_taxa(genres_transmis, ps16Ssoilrhizo_genre)
...
# Save
ggsave("your_path/Fig5_barplot_deseq_differentially_transmitted_gen0_gen1_soil_16S.svg", deseq_barplot, width = 6, height = 15)
...
ps16Sgen0
ps16Sgen0 <- prune_taxa(taxa_sums(ps16Sgen0) >= 1, ps16Sgen0)
ps16Sgen0 #35098 taxa and 54 samples



# Courbe de rarefaction avec limite de l'axe des y à 10^4
RC <- ggrare(ps16Sgen0, step = 100, se = FALSE, color = "Compartment") + 
  labs(title = "Rarefaction curve 16S, 
  ...
  theme(
    ...
    legend.text = element_text(size = 12)
      ...
      scale_color_manual(values = c("Leaf" = "#369686", "Root" = "#E3CD3B", "Rhizosphere" = "#55463A")) +  # Couleurs personnalisées  scale_x_continuous(lim16S = c(0, 75000))
...
ggsave(RC, dpi=300, device = svg, width = 7, height = 4, filename = "your_path/Supp_1_Rarefaction curve tout par compartiment gen0 16S.svg")




#subset dans tous les sens
ps16Sgen1 <- subset_samples(ps16S, Generation == "Gen1")
ps16Sgen1
ps16Sgen1 <- prune_taxa(taxa_sums(ps16Sgen1) >= 1, ps16Sgen1)
ps16Sgen1 #35098 taxa and 54 samples



# Courbe de rarefaction avec limite de l'axe des y à 10^4
RC <- ggrare(ps16Sgen1, step = 100, se = FALSE, color = "Compartment") + 
  labs(title = "Rarefaction curve 16S, Plants 
  ...
  theme(
    ...
    legend.text = element_text(size = 12)
      ...
      scale_color_manual(values = c( "Root" = "#E3CD3B", "Rhizo" = "#55463A")) +  # Couleurs personnalisées  scale_x_continuous(lim16S = c(0, 75000))
...
print(RC)
# Enregistrer le graphique
ggsave(RC, dpi=300, device = svg, width = 7, height = 4, filename = "your_path/Supp_1_Rarefaction curve tout par compartiment gen1 16S.svg")
